...
## gruplama  # aynı değerler gruplanır
personeller = {
...
result = df.groupby("Semt").get_group("Kadıköy") ## semtlere göre grupla kdıköy olanları getir
result = df.groupby("Departman").get_group("Muhasebe") ## departmana göre grupla muhasebe olanları getir
...
result = df.groupby("Semt")["Çalışan"].count() ## xemtlere göre kişi sayısı
result = df.groupby("Departman")["Yaş"].max()
result = df.groupby("Departman")["Maaş"].min()
result = df.groupby("Departman")["Maaş"].max()
result = df.groupby("Departman")["Maaş"].max()["Muhasebe"] ## sadece muhasebe departmanının max maaşı
# result = df.groupby("Departman").agg(np.mean) # çalışmıyor
