...
result = df[df["Salary"]==df["Salary"].max()] ## tüm satır
result = df[df["Salary"]==df["Salary"].max()]["Name"] ## index bilgisi isim sutunu
result = df[df["Salary"]==df["Salary"].max()]["Name"].iloc[0] ## sadece isim sutunu
...
result = df[df["Name"] == "John Holland"]   ## tüm satır
result = df[df["Name"] == "John Holland"]["Team"] ## index ve sutun 
result = df[df["Name"] == "John Holland"]["Team"].iloc[0] ## sadece sutun içeriği

# # 8- Takımlara göre oyuncuların ortalama maaş bilgisi nedir ?
# result = df.groupby("Team").mean()["Salary"] ## hata verdi
