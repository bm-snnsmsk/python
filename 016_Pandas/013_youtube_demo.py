...
# result = df.groupby("category_id").mean().sort_values("likes")["likes"] ## hata verdi

# # 11- Kategoriye göre yorum sayılarını yukarıdan aşağıya sıralayınız.
# result = df.groupby("category_id").sum().sort_values("comment_count", ascending = False)["comment_count"] ### hata verdiii
...
def likedislikeoranhesapla(dataset):
    ...
    liste = list(zip(likesList,dislikesList)) ## ziple sonra 
