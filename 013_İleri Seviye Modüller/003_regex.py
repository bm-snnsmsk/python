...
# result = re.findall("a", str) ## kaç tane kelime ?

# result = re.split(" ") ## boşluktan veya belirtilecek başka bir karakterden böler
# result = re.split("Sinan") ## boşluktan veya belirtilecek başka bir karakterden böler >> ['', ' Şimşek : Bilgisayar ', ' Mühendisi | Mardin İl Sağlık Müdürlüğü']
...
# result = result.group() ## Sinan    >>> re.search("Sinan", str) ile aramada çalışır
# result = result.string   ## tüm ifade >>> re.search("Sinan", str) ile aramada çalışır

result = re.findall("[abc]", str)   ## karakterler
result = re.findall("[a-e]", str)    ## a'dan e'ye arar
result = re.findall("[a-z]", str)    ## a'dan z'ye arar
...
result = re.findall("ma*n", str)  ## a 0 ya da daha fazla var mı  (mn, man, maaan olur ama main olmaz)
result = re.findall("ma+n", str)  ## a 1 ya da daha fazla var mı
result = re.findall("ma?n", str)  ## a 0 veya 1 tane var mı
result = re.findall("san{2}", str)  ## sa'dan sonra en az 2 tane n
result = re.findall("san{2, 5}", str)  ## sa'dan sonra en az 2 en fazla 5 tane n
...
result = re.findall("\A", str)  ## Sinan en başında
...
result = re.findall("\Bsnn", str)  ## kelimelerin başında snn yok mu
result = re.findall("snn\B", str)  ## kelimelerin sonunda snn yok mu
